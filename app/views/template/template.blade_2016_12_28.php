...
<div id="wrapper" class="container showLoader" style="display: none;">
    <header>
        <div class="row center-block">
            <div class="col-md-12" id="head-logo">
                ...
                <div class="playing col-lg-6 col-md-6 col-sm-12 col-xs-12">
                ...
                <div class="group1 col-lg-6 col-md-6 col-sm-12 col-xs-12">
                    <div class="header_group">
                        ...
                        <div class="input-group" style='float:left;width:35%;padding-left:10px;'>
                            {{ Form::text('tsearch', '', array('class' => 'form-control', 'placeholder' => 'Search', 'id' => 'tSearch', 'style' => 'height:32px;')) }}
                            <span class="input-group-btn">
                                <button type="button" class="btn" style='height:32px;padding:5px 15px;' onclick='OnSeachClicked()'><span class="fui-search"></span></button>
                            </span>
                        </div>

                        <img src='{{ URL::to('/') }}/images/help.png' style='width:28px;cursor:pointer;margin-left:10px;' title="Show Help" onclick="ShowHelp(1)">
                        <img style='margin-left:10px;width:60px;' src='/images/1studio_top_logo.png'>
                    ...
                    @if($status == 1)
                        <button class="btn btn-block btn-lg btn-success stream_status stop_stream" title="STOP STREAM" style='display:inline;'>
                            <i class="fa fa-stop"></i> &nbsp;STOP STREAM
                        </button>
                    @elseif($status == 0)
                        <button class="btn btn-block btn-lg btn-success stream_status start_stream" title="START STREAM" style='display:inline;'>
                            <i class="fa fa-play"></i> &nbsp;START STREAM
                        </button>
                    @endif
                    <button class="btn btn-block btn-lg btn-danger watch_now" title="WATCH NOW!" style='display:inline;'>
                        <i class="fa fa-arrow-circle-right"></i> &nbsp;WATCH NOW!
                    </button>
                    <button class="btn" title="Situation Room" style='display:inline;background:#3289c7;color:#fff;height: 50px;width: 32%;font-size: 19px;' onclick='OpenSituationRoom()'>
                        Situation Room
                    </button>
    ...
    <nav>
        <div class="row center-block">
            <div class="container">
                <div id="nav" class="row col-md-12">
                    @if(isset($channel))

                        <div class="btn-group btn-breadcrumb">
                            @if(Auth::user()->is(User::USER_MANAGE_MEDIA))
                                <a class="btn btn-primary" href="../channel_{{ $channel['id'] }}/videos"><!--i class="fa fa-video-camera"></i-->Videos</a>
                                <a class="btn btn-primary" href="../channel_{{ $channel['id'] }}/collections"><!--i class="fa fa-film"></i-->Categories</a>
                                <a class="btn btn-primary" href="../channel_{{ $channel['id'] }}/playlists"><!--i class="fa fa-play-circle"></i-->Live Playlists</a>
                                <a class="btn btn-primary" href="/channel_{{ $channel['id'] }}/schedule">Schedule</a>
                                <a class="btn btn-primary" href="../channel_{{ $channel['id'] }}/tvapp_playlists"><!--i class="fa fa-play-circle"></i-->Channel Manager</a>
                                <a class="btn btn-primary" href="../channel_{{ $channel['id'] }}/Analytics"><!--i class="fa fa-play-circle"></i -->Analytics</a>
                            @endif
                            @if(Auth::user()->is(User::USER_MANAGE_CHANNEL))
                                <a class="btn btn-primary" href="../channel_{{ $channel['id'] }}/settings"><i class="fa fa-cog"></i></a>
                            @endif
                        </div>
                    @endif
                </div>
