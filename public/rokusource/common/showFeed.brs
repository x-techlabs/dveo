...
Function parse_show_feed(xml As Object, feed As Object) As Void
    ...
    for each curShow in showList
        ...
        item.viewing          = validstr(curShow@viewing) 
        item.subscription     = validstr(curShow@subscription) 
        ...
        item.Runtime          = validstr(curShow.runlength.GetText())
        ...
        if (item.HDPosterUrl.instr(".jpg")=0 and item.HDPosterUrl.instr(".png")=0) then
            item.HDPosterUrl = item.HDPosterUrl + ".jpg"
        endif

        if (item.SDPosterUrl.instr(".jpg")=0 and item.SDPosterUrl.instr(".png")=0) then
            item.SDPosterUrl = item.SDPosterUrl + ".jpg"
        endif
